-- https://gist.github.com/2bt/3869745

--local isDown = love.keyboard.isDown
local isDown = love.mousepressed
...
love.mousepressed(x, y, button, istouch)
	...
	if math.abs(ball.y) >= 285 then ball.vy = -ball.vy end
		...
		if ball.x < -415 then  -- se elemento bola estiver na posição x inferior a -415
			ball.x = 0
			ball.vx = -ball.vx
			score2 = score2 + 1
		end
...
function love.mousepressed(x, y, button, istouch)
   if button == 1 and y > pad1_y and y < pad1_y then -- Versions prior to 0.10.0 use the MouseConstant 'l'
      pady = pady  + y
   end
end
